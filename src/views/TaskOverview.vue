...
const stats = computed(() => {
  ...
  const loaded = boxes.filter((b) => statusLabel(b.status) === '已装车' || statusLabel(b.status) === '已到达').length;
  const unpacked = boxes.filter((b) => statusLabel(b.status) === '已到达' || statusLabel(b.status) === '已拆箱').length;
  const damaged = boxes.filter((b) => statusLabel(b.status) === '破损' || statusLabel(b.status) === '缺失').length;
