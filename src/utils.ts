...
export interface BoxStatusCount {
  total: number;
  loaded: number;
  unpacked: number;
  damaged: number;
...
export function countBoxesByStatus(boxes: MoveTask['boxes']): BoxStatusCount {
  const counts: BoxStatusCount = { total: boxes.length, loaded: 0, unpacked: 0, damaged: 0 };
  for (const box of boxes) {
    const label = statusLabel(box.status);
    if (label === '已装车' || label === '已到达') {
      counts.loaded += 1;
    }
    if (label === '已到达' || label === '已拆箱') {
      counts.unpacked += 1;
    }
    if (label === '破损' || label === '缺失') {
      counts.damaged += 1;
    }
  }
  return counts;
...
export function roomProgress(task: MoveTask, room: string): { total: number; unpacked: number; damaged: number } {
  ...
  const counts = countBoxesByStatus(boxes);
  return { total: counts.total, unpacked: counts.unpacked, damaged: counts.damaged };
}
